...
export class PerformanceProfiler {
  ...
  recordFrame(durationMs) {
    ...
    this.metrics.fps = Math.min(
      120,
      Math.round(1000 / Math.max(1, this.metrics.avgFrameMs)),
    );
  ...
  updateMemoryMetrics() {
    if (
      ...
      this.metrics.heapLimitMB = (mem.jsHeapSizeLimit / (1024 * 1024)).toFixed(
        0,
      );
    } else {
      this.metrics.memoryMB = (((performance.now() * 0.01) % 50) + 30).toFixed(
        1,
      ); // fallback estimation
  ...
  async runFullBenchmarkSuite(onProgress = null) {
    ...
    try {
      ...
      updateStatus(
        1,
        10,
        "Тестування роздільної здатності (256, 512, 1024)...",
      );
      ...
      updateStatus(
        2,
        40,
        "Тестування під високим навантаженням (5, 10, 20 шарів)...",
      );
        ...
        for (let i = 0; i < count; i++) {
          ...
          this.state.layers.push({
            ...
            params: {
              ...
              mode: "f1",
              warps:
                i % 4 === 0 ? [{ type: "vortex", strength: 20, freq: 4 }] : [],
            },
          });
        }
      ...
      updateStatus(
        3,
        80,
        "Стрес-тест активної деформації та генерації (30 параметрів)...",
      );
      ...
      const fps10Layers = report.multiLayerTest["10_layers"]
        ? report.multiLayerTest["10_layers"].fps
        : 30;
