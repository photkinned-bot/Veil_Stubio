...
export class SyncManager {
  ...
  subscribe(callback) {
    if (
      ...
      !this.subscribers.includes(callback)
    ) {
  ...
  pullCanvasData() {
    try {
      ...
      if (typeof window.renderProject === "function") {
        const res =
          window.mapGeneratorTab?.targetResolution ||
          (window.state && window.state.resolution) ||
          512;
  ...
  extractActiveLayerImageData() {
    ...
    const layer =
      window.state.layers.find((l) => l.id === selectedId) ||
      window.state.layers[0];
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          this.notifySubscribers(imgData, {
            source: "manual",
            filename: file.name,
          });
          resolve(imgData);
