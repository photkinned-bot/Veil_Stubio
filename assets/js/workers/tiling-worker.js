...
self.onmessage = function (e) {
  ...
  if (action === "PROCESS_TOROIDAL_GUARD") {
    ...
    self.postMessage({ action, id, buffer: pixels.buffer, width, height }, [
      pixels.buffer,
    ]);
  } else if (action === "GENERATE_PERLIN_NOISE") {
    const resultBuffer = generatePerlinNoiseBuffer(width, height, params);
    self.postMessage({ action, id, buffer: resultBuffer, width, height }, [
      resultBuffer,
    ]);
  }
};
    ...
    for (let x = 0; x < guardW; x++) {
      ...
      for (let c = 0; c < 3; c++) {
        ...
        if (preserveDetail > 0) {
          const detailL =
            valL -
            (copy[getPixelIdx(w, h, x - 1, y) + c] +
              copy[getPixelIdx(w, h, x + 1, y) + c]) /
              2;
          const detailR =
            valR -
            (copy[getPixelIdx(w, h, w - 1 - x - 1, y) + c] +
              copy[getPixelIdx(w, h, w - 1 - x + 1, y) + c]) /
              2;
    ...
    for (let y = 0; y < guardW; y++) {
      ...
      for (let c = 0; c < 3; c++) {
        ...
        if (preserveDetail > 0) {
          const detailT =
            valT -
            (copy[getPixelIdx(w, h, x, y - 1) + c] +
              copy[getPixelIdx(w, h, x, y + 1) + c]) /
              2;
          const detailB =
            valB -
            (copy[getPixelIdx(w, h, x, h - 1 - y - 1) + c] +
              copy[getPixelIdx(w, h, x, h - 1 - y + 1) + c]) /
              2;
...
function generatePerlinNoiseBuffer(w, h, params) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const idx = (y * w + x) * 4;
      const n = Math.floor(
        (Math.sin(x * scale) * Math.cos(y * scale) + 1) * 127.5,
      );
      pixels[idx] = n;
      pixels[idx + 1] = n;
      pixels[idx + 2] = n;
